...
To deploy a model, first use one of the notebooks to produce a `model.keras` file, and save a `train_data.csv` containing *unscaled* features. The latter is optional, but it can be faster, as there is a handy script to make the required header file.
